...
Mac / Windows アプリ版 (packaging/prp_path_fixer.spec で PyInstaller ビルド) の本体。
...
def fix_file(
    ...
    if inplace:
        ...
        dest = default_output_path(src, target)
...
def _log_result(src: Path, dest: Optional[Path], result: FixResult, log) -> None:
    if dest is None:
...
def process(
    ...
    for src in files:
        try:
            dest, result = fix_file(src, target, inplace, all_text)
            _log_result(src, dest, result, log)
...
def run_gui(initial: Optional[List[Path]] = None) -> int:
    try:
        ...
        print("tkinter が使えないため GUI を起動できません。")
        return 1
    ...
    def run() -> None:
        if not files:
            ...
            return
        errors = process(files, target.get(), inplace=inplace.get(), all_text=all_text.get(), log=log)
...
def main() -> int:
    # アイコンにドロップされたファイル (Windows は引数、Mac は OpenDocument) を読み込んだ状態で開く
    return run_gui([Path(a) for a in sys.argv[1:] if not a.startswith("-psn")])
